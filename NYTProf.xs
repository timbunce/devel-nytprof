...
#define END_OUTPUT_PID(pid) STMT_START { \
    assert(out != NULL); output_tag_int(NYTP_TAG_PID_END, pid); NYT_flush(out); \
} STMT_END
...
static int
NYT_close(NYT_file file, int discard) {
...
output_nv(NV nv)
{
    NYT_write(out, (unsigned char *)&nv, sizeof(NV));
}
...
reinit_if_forked(pTHX)
{
     ...
     */
    NYT_close(out, 1); /* 1: discard output, to stop it being flushed to disk */
